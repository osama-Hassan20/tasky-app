        ...
        cubit.tasksModel.isNotEmpty
            ? ListView.builder(
              controller: scrollController,
                ...
                itemCount: widget.cubit.isLoadDown ? cubit.tasksModel.length + 1 : cubit.tasksModel.length,
                ...
                child: const Center(
                  child: CustomLoadingAnimation(color: Colors.amber,)
                    ...
                    print('ubit.tasksModel[index].status = ${cubit.tasksModel[index].status} == cubit.status = ${cubit.status}');
                  if(cubit.status == 'tasks'){
                    return DefaultItem(
                    taskModel: cubit.tasksModel[index],
                  );
                  }
                  else if (cubit.tasksModel[index].status == cubit.status) {
                    cubit.counter++;
                    ...
                    return cubit.counter ==0 && index == cubit.tasksModel.length -1?
